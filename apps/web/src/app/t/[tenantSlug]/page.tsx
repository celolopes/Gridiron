    ...
    <div className="container mx-auto px-4 space-y-12">
      ...
      <section>
        <div className="flex items-center justify-between mb-8">
          ...
          <a href={`/t/${tenantSlug}/products`} className="text-accent hover:underline font-medium">
            Ver todos
          </a>
        </div>
          ...
          {products.slice(0, 4).map((product: any) => (
            <a key={product.id} href={`/t/${tenantSlug}/products/${product.slug || product.id}`}>
              <GlassCard className="group cursor-pointer hover:shadow-2xl hover:-translate-y-1 p-4 border border-glass-border">
                ...
                <p className="text-muted-foreground font-medium">{new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(product.price)}</p>
