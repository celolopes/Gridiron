...
export default async function TenantHome({ params }: { params: Promise<{ tenantSlug: string }> }) {
  ...
  return (
    <div className="pb-24">
      ...
      <section className="relative min-h-[100svh] md:min-h-[90vh] flex items-center pt-32 pb-16 md:pt-20 md:pb-0 overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent md:bg-gradient-to-t md:from-black/80 md:via-transparent md:to-black/30 z-10" />
        </div>

        <div className="container mx-auto px-4 sm:px-6 relative z-20">
          <div className="max-w-3xl">
            <div className="inline-flex items-center space-x-2 bg-accent/20 border border-accent/20 px-3 py-1.5 rounded-full mb-6 mt-12 md:mt-0">
              <span className="w-2 h-2 bg-accent rounded-full animate-pulse" />
              <span className="text-accent text-[10px] sm:text-xs font-black uppercase tracking-[0.2em] whitespace-nowrap">New Season 2024/25</span>
            </div>
            <h1 className="text-5xl sm:text-6xl md:text-8xl font-black italic uppercase tracking-tighter text-white leading-[0.85] mb-6 md:mb-8">
              The <span className="text-accent">Elite</span>
              <br />
              Collection
            </h1>
            <p className="text-base sm:text-lg md:text-xl text-zinc-300 mb-8 md:mb-10 max-w-xl font-medium leading-relaxed drop-shadow-md">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <Link href={`/t/${tenantSlug}/products`} className="w-full sm:w-auto">
                <GlassButton
                  size="lg"
                  className="w-full px-10 rounded-full font-black uppercase tracking-widest text-[13px] sm:text-sm bg-white text-black hover:bg-accent hover:text-white transition-all border-none py-4 sm:py-3"
                >
                  Comprar Agora
                </GlassButton>
              </Link>
              <Link href={`/t/${tenantSlug}/products?category=jerseys`} className="w-full sm:w-auto">
                <GlassButton
                  variant="secondary"
                  size="lg"
                  className="w-full px-10 rounded-full font-black uppercase tracking-widest text-[13px] sm:text-sm border-white/20 hover:border-white transition-all text-white py-4 sm:py-3"
                >
                  Ver Jerseys
                </GlassButton>
              </Link>
      ...
      <section className="relative z-20 -mt-8 md:-mt-16 mb-20 px-4 sm:px-6">
        <div className="container mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 p-6 md:p-8 rounded-3xl md:rounded-[40px] bg-black/60 md:bg-white/80 md:dark:bg-zinc-900/80 backdrop-blur-2xl border border-white/10 md:border-white/20 md:dark:border-white/5 shadow-2xl overflow-hidden relative group">
            ...
            {[
              { icon: <ShieldCheck className="w-5 h-5 md:w-6 md:h-6" />, title: "100% Original", desc: "Oficial & Certificado" },
              { icon: <Truck className="w-5 h-5 md:w-6 md:h-6" />, title: "Envio Rápido", desc: "Todo o Brasil" },
              { icon: <Star className="w-5 h-5 md:w-6 md:h-6" />, title: "Premium VIP", desc: "Qualidade de Elite" },
              { icon: <ShoppingBag className="w-5 h-5 md:w-6 md:h-6" />, title: "Compra Segura", desc: "Dados Protegidos" },
            ].map((benefit, i) => (
              <div key={i} className="flex flex-col items-center md:items-start text-center md:text-left group/item relative z-10 transition-transform hover:scale-105 duration-300">
                <div className="mb-3 md:mb-4 p-2.5 md:p-3 rounded-2xl bg-accent/10 text-accent group-hover/item:bg-accent group-hover/item:text-white shadow-lg shadow-accent/5 transition-all duration-500">
                  {benefit.icon}
                </div>
                <h3 className="text-[11px] md:text-xs font-black uppercase tracking-tighter text-zinc-100 md:text-zinc-900 md:dark:text-zinc-100 group-hover/item:text-accent transition-colors">
                  {benefit.title}
                </h3>
